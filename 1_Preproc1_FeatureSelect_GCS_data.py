...
def parse_args() -> tuple[argparse.Namespace, List[str]]:
    parser = argparse.ArgumentParser(
        description="Filter the GSC CSV to keep latitude/longitude and selected feature columns.",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path for the filtered CSV file (defaults to alongside the input CSV).",
    )
    parser.add_argument(
        "--features",
        nargs="+",
        required=True,
        help="Feature column names to retain in the output CSV.",
    )
    parser.add_argument(
        "--special-columns1",
        dest="special_columns1",
        nargs="+",
        default=[],
        help="Dictionary indicator columns to collapse into lithology dictionary layers (following Lawley et al., 2021).",
    )
    parser.add_argument(
        "--lat-column",
