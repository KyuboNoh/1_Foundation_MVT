...
def _normalize_prediction_result(result: Any, default_reference: Optional[Any]) -> Dict[str, Tuple[np.ndarray, np.ndarray, Any]]:
    ...
    if isinstance(result, dict):
        for key, value in result.items():
            if isinstance(value, dict):
                ...
                context = {k: v for k, v in value.items() if k not in {"mean", "std"}}
            else:
                mean_map, std_map = value
                context = None
            if mean_map is None or std_map is None:
                continue
            normalized[str(key)] = (mean_map, std_map, context)
    else:
        mean_map, std_map = result
        normalized["GLOBAL"] = (mean_map, std_map, None)

    return normalized


def _try_extract_masks(reference: Any) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
    if reference is None:
        return None, None
    if isinstance(reference, dict):
        return reference.get("valid_mask"), reference.get("boundary_mask")
    if hasattr(reference, "srcs"):
        return _extract_valid_boundary_masks(reference)
    if hasattr(reference, "dataset_mask"):
        try:
            dataset_mask = reference.dataset_mask()
        except Exception:
            dataset_mask = None
        if dataset_mask is not None:
            mask_arr = np.asarray(dataset_mask)
            if mask_arr.ndim == 3:
                mask_arr = mask_arr[0]
            dataset_bool = mask_arr != 0
            return dataset_bool, _compute_boundary_mask(dataset_bool)
    return None, None


def _resolve_reference_context(
    context: Any,
    default_reference: Optional[Any],
) -> Tuple[Optional[Any], Optional[np.ndarray], Optional[np.ndarray]]:
    reference = None
    ...
    if isinstance(context, dict):
        ...
        boundary_mask = context.get("boundary_mask")
        if context.get("stack") is not None:
            reference = context["stack"]
        elif context.get("reference") is not None:
            reference = context["reference"]
        elif any(key in context for key in ("profile", "transform", "crs")):
            ...
            if "transform" in context:
                profile["transform"] = context["transform"]
            if "crs" in context:
                profile["crs"] = context["crs"]
            reference = {
                "profile": profile,
    ...
    ref_valid, ref_boundary = _try_extract_masks(reference)
    if valid_mask is None:
        valid_mask = ref_valid
    if boundary_mask is None:
        boundary_mask = ref_boundary

    # Ensure dict references capture updated masks
    if isinstance(reference, dict):
        ...
        if boundary_mask is not None:
            reference = dict(reference)
            reference["boundary_mask"] = boundary_mask
...
def write_prediction_outputs(
    ...
    neg_coords_by_region: Optional[Dict[str, List[Tuple[int, int]]]] = None,
) -> None:
    normalized = _normalize_prediction_result(result, default_reference)
    if not normalized:
    ...
    for region_name, (mean_map, std_map, context) in normalized.items():
        if mean_map is None or std_map is None:
            continue

        reference_obj, valid_mask, boundary_mask = _resolve_reference_context(
            context,
            default_reference=default_reference,
        )
        if reference_obj is None:
            raise RuntimeError(
                "Prediction reference metadata is missing; provide a stack, raster profile, or explicit "
                "reference dictionary with 'profile' information."
            )
        ...
        pos_positions: Optional[List[Tuple[int, int]]] = None
        if pos_coords_by_region:
            pos_positions = pos_coords_by_region.get(region_name) or pos_coords_by_region.get("GLOBAL")
            if pos_positions:
                ...
                pos_positions = filtered_positions or None

        neg_positions: Optional[List[Tuple[int, int]]] = None
        if neg_coords_by_region:
            neg_positions = neg_coords_by_region.get(region_name) or neg_coords_by_region.get("GLOBAL")
            if neg_positions:
                filtered_positions: List[Tuple[int, int]] = []
                h, w = masked_mean.shape
                for r, c in neg_positions:
                    if 0 <= r < h and 0 <= c < w:
                        if valid_mask is None or valid_mask[r, c]:
                            filtered_positions.append((r, c))
                neg_positions = filtered_positions or None
        ...
        save_geotiff(str(mean_path), masked_mean, reference_obj, valid_mask=valid_mask)
        save_geotiff(str(std_path), masked_std, reference_obj, valid_mask=valid_mask)
...
def save_geotiff(path, array, ref_src, valid_mask: np.ndarray = None, nodata: float = np.nan):
    if hasattr(ref_src, "profile"):
        ...
        raise TypeError(
            "save_geotiff expected a rasterio dataset (with .profile) or a profile dictionary."
        )
...
def save_png(
    ...
    *,
    ...
    def fill_nans_inside_boundary(arr, boundary):
        arr = np.asarray(arr, dtype=np.float32)
        mask_inside = boundary.astype(bool)
        missing_inside = np.isnan(arr) & mask_inside
        filled = np.nan_to_num(arr, nan=0.0)
        filled_interp = inpaint_biharmonic(filled, missing_inside, channel_axis=None)
        filled_interp[~mask_inside] = np.nan
    ...
    if valid_mask is not None and valid_mask.shape == display.shape:
        display = np.where(valid_mask, display, np.nan)

    if boundary_mask is not None and boundary_mask.shape == display.shape:
        display = fill_nans_inside_boundary(display, boundary_mask)
    ...
    if neg_coords:
        coords_arr = np.asarray(neg_coords, np.float32)
        if coords_arr.ndim == 2 and coords_arr.size:
            ys = np.clip(coords_arr[:, 0] + 0.5, 0, height - 0.5)
            xs = np.clip(coords_arr[:, 1] + 0.5, 0, width - 0.5)
            ax.scatter(
