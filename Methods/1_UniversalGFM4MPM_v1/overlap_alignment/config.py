...
def load_config(path: Path) -> AlignmentConfig:
    ...
    if integration_dir is not None:
        candidate_dirs = [integration_dir / "data" / "Pairs", integration_dir / "data"]
        for pairs_dir in candidate_dirs:
            if overlap_pairs_value is None and pairs_dir.exists():
                for candidate in sorted(pairs_dir.glob("*overlap_pairs.json")):
                    if "positive" not in candidate.stem:
                        overlap_pairs_value = str(candidate)
                        break
            if overlap_pairs_aug_value is None and pairs_dir.exists():
                positive_match = _first_match(pairs_dir, ["*overlap_pairs_positive.json", "*overlap_pairs_augmented.json"])
                if positive_match is not None:
                    overlap_pairs_aug_value = str(positive_match)
            if overlap_pairs_value and overlap_pairs_aug_value:
                break
